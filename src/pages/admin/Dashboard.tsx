...
export function Dashboard() {
  ...
  const stats = [
    { name: 'Receita Total', value: 'R$ 45.231,89', change: '+20.1%', icon: DollarSign },
  ...
  const recentOrders = [
    { id: '#1234', customer: 'Maria Silva', date: 'Hoje, 10:23', total: 'R$ 359,90', status: 'Pago' },
    { id: '#1233', customer: 'João Souza', date: 'Hoje, 09:12', total: 'R$ 1.250,00', status: 'Aguardando Pagamento' },
    { id: '#1232', customer: 'Ana Paula', date: 'Ontem, 16:45', total: 'R$ 89,90', status: 'Enviado' },
    { id: '#1231', customer: 'Carlos Eduardo', date: 'Ontem, 14:20', total: 'R$ 450,50', status: 'Entregue' },
  ];
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
        {stats.map((stat) => (
          <div key={stat.name} className="bg-white rounded-2xl border border-neutral-200/40 p-6 shadow-[0_2px_10px_rgba(0,0,0,0.03)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.06)] transition-all duration-300">
            <div className="flex items-center justify-between mb-6">
              ...
              <span className={`text-[11px] font-semibold px-2.5 py-1 rounded-full ${stat.change.startsWith('+') ? 'bg-emerald-50 text-emerald-600' : 'bg-neutral-100 text-neutral-600'}`}>
                {stat.change}
              </span>
            </div>
            <div>
              <p className="text-[11px] font-semibold uppercase tracking-widest text-neutral-400 mb-1">{stat.name}</p>
              <h3 className="text-[28px] font-medium text-neutral-900 tracking-tight">{stat.value}</h3>
            </div>
          </div>
        ))}
      </div>
                ...
                <tbody className="divide-y divide-neutral-100/60">
                  {recentOrders.map((order) => (
                    <tr key={order.id} className="hover:bg-neutral-50/50 transition-colors group cursor-pointer">
                      ...
                      <td className="py-4 px-6 font-medium text-[13px] text-neutral-900 text-right">{order.total}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <div className="bg-white rounded-2xl border border-neutral-200/40 shadow-[0_2px_10px_rgba(0,0,0,0.03)] p-6 flex flex-col justify-between">
          ...
          <div className="mt-8 pt-6 border-t border-neutral-100/60">
             <div className="bg-[#F5F5F7] rounded-xl p-5 border border-neutral-200/50 relative overflow-hidden">
                ...
                <a href="#" className="text-[11px] font-semibold text-blue-600 uppercase tracking-widest hover:text-blue-700 hover:underline relative z-10">Falar com Suporte &rarr;</a>
             </div>
          </div>
        </div>
      </div>
